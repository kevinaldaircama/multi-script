...
if [[ "${1:-}" == "--auto" ]]; then
    ...
    echo
    echo -e "${CYAN}${BOLD}🚀 INSTALACIÓN AUTOMÁTICA BHTTP${RESET}"
    echo

    # Ejecutamos el instalador original.
    # Sin --puerto, para que bhttp-install.sh pueda elegir
    # automáticamente un puerto libre.
    bash "$INSTALL"
    ...
    if [[ "$RC" -ne 0 ]]; then
        echo
        echo -e "${RED}${BOLD}✘ Error instalando BHTTP.${RESET}"
        exit "$RC"
    fi
    ...
    if systemctl is-active --quiet bhttp 2>/dev/null; then
        ...
        echo -e "${GREEN}${BOLD}✔ BHTTP instalado y activo.${RESET}"

        CONFIG_FILE="$BASE/config.conf"

        if [[ -f "$CONFIG_FILE" ]]; then
            if grep -q '^BHTTP=' "$CONFIG_FILE"; then
                sed -i 's/^BHTTP=.*/BHTTP=ON/' "$CONFIG_FILE"
            else
                echo 'BHTTP=ON' >> "$CONFIG_FILE"
            fi
        else
            mkdir -p "$BASE"
            echo 'BHTTP=ON' > "$CONFIG_FILE"
        ...
        echo -e "${GREEN}✔ Estado guardado: BHTTP=ON${RESET}"

        PUERTO_AUTO="$(
            systemctl cat bhttp 2>/dev/null |
            grep -oE -- '--port [0-9]+' |
            tail -1 |
            awk '{print $2}'
        )"

        if [[ -n "$PUERTO_AUTO" ]]; then
            echo -e "${GREEN}✔ Puerto BHTTP: $PUERTO_AUTO${RESET}"
        fi
        ...
        echo
        echo -e "${RED}${BOLD}✘ BHTTP fue instalado pero no está activo.${RESET}"
        echo
