...
load_host() {
    ...
    CONFIG_DOMAIN="$(echo "$CONFIG_DOMAIN" | xargs)"
    ...
    if [[ -f /etc/xray/domain ]]; then
        ...
        FILE_DOMAIN=$(cat /etc/xray/domain 2>/dev/null)

        FILE_DOMAIN="$(echo "$FILE_DOMAIN" | xargs)"
...
install_dependencies() {
    ...
    apt-get install -y \
        ...
        error_msg "No se pudieron instalar dependencias."
...
# ==============================================================
# DETECTAR INBOUND SIN PUERTO
# ==============================================================

find_invalid_inbounds() {
...
validate_xray_config() {
    ...
    if ! validate_json_file "$XRAY_CFG"; then
...
validate_temp_config() {
    ...
    # MUY IMPORTANTE
    #
    # El archivo debe terminar en .json.
    # ----------------------------------------------------------
...
show_config_problem() {
    ...
    INVALID=$(find_invalid_inbounds "$FILE")
    ...
    if [[ "$INVALID" != "[]" &&
        ...
        warning "Se detectó un inbound sin puerto."
        ...
        echo -e \
            "${YELLOW}Ese inbound debe tener un puerto válido.${RESET}"
...
install_xray() {
    ...
    if [[ -f "$XRAY_CFG" ]]; then
        ...
        if validate_json_file "$XRAY_CFG"; then
            ...
            if validate_xray_config; then
                ...
                ok "Configuración actual válida."

            else

                warning "La configuración actual tiene problemas."

                show_config_problem "$XRAY_CFG"

                echo

                warning "No se modificará automáticamente."
            ...
            warning "No se reemplazará automáticamente."

        fi

    else

        info "Creando configuración base..."
    ...
    if validate_xray_config; then
        ...
        error_msg "La configuración actual no es aceptada por Xray."
...
create_user() {
    ...
    if ! validate_temp_config "$TEMP"; then
        ...
        error_msg "La cuenta NO fue aplicada."
...
create_all_users() {
    ...
    read -rp \
        "$(echo -e "${YELLOW}¿Crear los 4 protocolos? [S/n]: ${RESET}")" \
        CONFIRM
    ...
    echo -e \
        "${CYAN}${BOLD}VLESS${RESET}"

    echo -e \
        "UUID: ${YELLOW}$VLESS_ID${RESET}"

    echo

    echo -e \
        "${CYAN}${BOLD}VMess${RESET}"

    echo -e \
        "UUID: ${YELLOW}$VMESS_ID${RESET}"

    echo

    echo -e \
        "${CYAN}${BOLD}Trojan${RESET}"

    echo -e \
        "Password: ${YELLOW}$TROJAN_ID${RESET}"

    echo

    echo -e \
        "${CYAN}${BOLD}gRPC${RESET}"

    echo -e \
        "UUID: ${YELLOW}$GRPC_ID${RESET}"
    ...
    echo -e \
        "${WHITE}Creados:${RESET} ${GREEN}$CREATED${RESET}"
...
list_users() {
    ...
    if [[ "$TOTAL" -eq 0 ]]; then
        ...
        echo -e \
            "${YELLOW}No existen usuarios.${RESET}"
...
remove_all_users() {
    ...
    echo -e \
        "${RED}${BOLD}⚠ Se eliminará de VLESS, VMess, Trojan y gRPC.${RESET}"
...
diagnostic() {
    ...
    if [[ -n "$HOST" ]]; then
        ...
        error_msg "No se pudo obtener host."
    fi
    ...
    echo -e "${WHITE}Inbounds actuales:${RESET}"

    jq -r '
        .inbounds[]? |
        "  \(.protocol // "-") | listen=\(.listen // "-") | port=\(.port // "-") | tag=\(.tag // "-")"
    ' \
    "$XRAY_CFG" 2>/dev/null
...
restart_xray() {
    ...
    if ! validate_xray_config; then
        ...
        error_msg "No se puede reiniciar: configuración inválida."
...
create_account_menu() {
    ...
    while true; do
        ...
        case "$OP" in
            ...
            1)
                create_and_show_single "vless"
                ;;

            2)
                create_and_show_single "vmess"
                ;;

            3)
                create_and_show_single "trojan"
                ;;

            4)
                create_and_show_single "grpc"
                ;;
