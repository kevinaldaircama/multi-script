...
if dropbear_instalado; then  
    PROTOCOLOS+=("Dropbear|${DROPBEAR_PORTS}")  
fi  
  
if ssl_instalado; then  
    PROTOCOLOS+=("SSL/TLS|${SSL_PORTS}")  
fi  
  
if badvpn_instalado; then  
    PROTOCOLOS+=("BadVPN|${BADVPN_PORTS}")  
fi  
...
if (( TOTAL_PROTO == 0 )); then  
    ...
    for ((i=0; i<TOTAL_PROTO; i+=2)); do  
        ...
        if [[ -n "$PUERTO1" ]]; then  
            ...
            printf "   ${GREEN}[✓]${RESET} ${WHITE}${ICONO1} %-13s${RESET} ${GRAY}(%s)${RESET}" \  
                "$NOMBRE1" "$PUERTO1"  
  
        else  
  
            printf "   ${GREEN}[✓]${RESET} ${WHITE}${ICONO1} %-13s${RESET}" \  
                "$NOMBRE1"  
        ...
        if (( i + 1 < TOTAL_PROTO )); then  
            ...
            printf "    ${GREEN}[✓]${RESET} ${WHITE}${ICONO2} %-13s${RESET}" \  
                "$NOMBRE2"  
