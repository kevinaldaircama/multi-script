...
for line in s.splitlines(True):
 if line.strip().startswith('bind abns@haproxy-https') and 'ssl' in line and Path(pem).is_file() and pem not in line: line=line.rstrip('\n')+' crt '+pem+'\n'
 out.append(line)
...
ensure_cert(){ local domain="$1"; mkdir -p "$ACME_ROOT/.well-known/acme-challenge"; systemctl stop "$ACME_SERVICE" >/dev/null 2>&1 || true; if ss -ltnp 2>/dev/null | grep -q ":$ACME_PORT "; then fuser -k "$ACME_PORT/tcp" >/dev/null 2>&1 || true; fi; systemctl start "$ACME_SERVICE" || return 1; systemctl is-active --quiet "$ACME_SERVICE" || { journalctl -u "$ACME_SERVICE" -n 30 --no-pager; return 1; }; printf 'KEVINTECH-ACME-OK\n' > "$ACME_ROOT/.well-known/acme-challenge/installer-test"; if ! curl -fsS --max-time 15 "http://$domain/.well-known/acme-challenge/installer-test" | grep -qx 'KEVINTECH-ACME-OK'; then echo "✘ ACME no responde por http://$domain/.well-known/acme-challenge/"; echo "Comprueba DNS, HAProxy y el puerto 80."; return 1; fi; if [[ -s "/etc/letsencrypt/live/$domain/fullchain.pem" && -s "/etc/letsencrypt/live/$domain/privkey.pem" ]]; then :; else command -v certbot >/dev/null 2>&1 || { apt-get update -y && apt-get install -y certbot || return 1; }; local email; read -rp "Correo para Let's Encrypt: " email; [[ -n "$email" ]] || return 1; certbot certonly --webroot -w "$ACME_ROOT" -d "$domain" --non-interactive --agree-tos --no-eff-email -m "$email" || return 1; fi; cat "/etc/letsencrypt/live/$domain/fullchain.pem" "/etc/letsencrypt/live/$domain/privkey.pem" > "$PEM"; chmod 600 "$PEM"; write_domain_config "$domain"; write_renew_hook; }
...
install_or_update(){ root_check; [[ -f "$WEB/server.py" ]] || { echo -e "${red}✘ Falta $WEB/server.py${reset}"; return 1; }; mkdir -p "$DATA" "$WEB/templates" "$ACME_ROOT/.well-known/acme-challenge"; local existing_domain="$(python3 -c 'import json,sys; p=sys.argv[1];
try: print(json.load(open(p)).get("server_domain",""))
except: print("")' "$DATA/config.json")"; existing_domain="${existing_domain:-$(current_domain)}"; if [[ -z "$existing_domain" ]]; then prompt_fresh || return 1; save_credentials "$WEB_ADMIN_USER" "$WEB_ADMIN_PASS" "$WEB_DOMAIN" || return 1; else WEB_DOMAIN="$existing_domain"; echo -e "${cyan}Actualizando sin cambiar usuario, contraseña ni dominio.${reset}"; fi; write_service; write_acme_service; systemctl daemon-reload; systemctl enable "$SERVICE" "$ACME_SERVICE" >/dev/null 2>&1 || true; systemctl stop "$ACME_SERVICE" >/dev/null 2>&1 || true; if ss -ltnp 2>/dev/null | grep -q ":$ACME_PORT "; then fuser -k "$ACME_PORT/tcp" >/dev/null 2>&1 || true; fi; systemctl start "$ACME_SERVICE" || return 1; patch_haproxy "$WEB_DOMAIN" || return 1; ensure_cert "$WEB_DOMAIN" || return 1; patch_haproxy "$WEB_DOMAIN" || return 1; systemctl restart "$ACME_SERVICE"; systemctl reset-failed "$SERVICE" >/dev/null 2>&1 || true; systemctl restart "$SERVICE"; sleep 1; if systemctl is-active --quiet "$SERVICE"; then echo -e "${green}✔ KevinTech Web activa.${reset}"; echo -e "${cyan}✔ Panel: https://$WEB_DOMAIN/login${reset}"; else journalctl -u "$SERVICE" -n 40 --no-pager; return 1; fi; }
